...
router.post("/button", async (req: Request, res: Response) => {
  try {
    const { phoneNumber, text, footer, buttons } = req.body;
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Phone number, text and at least one button are required" });
    }
    ...
    if (!result) {
      return res.status(500).json({
        success: false,
        error: "Failed to send button message, no response from WhatsApp",
      });
    }
    ...
    res.status(500).json({
      ...
      note: "Button messages may not be supported in all WhatsApp versions",
    });
  }
});
